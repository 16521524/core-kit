...
interface UnitCardProps {
  ...
  isSelected?: boolean;
  onSelect?: (unit: ApartmentUnit) => void;
}
  ...
  return (
    <TooltipProvider>
          ...
          <Tooltip>
                ...
                >
                  <div
                    className={`w-full h-full cursor-pointer transition-all duration-300 ${
                      theme.statusColors[unit.status]
                    } relative group hover:shadow-2xl hover:scale-[1.02] hover:z-10 rounded-xl p-2 flex flex-col items-center justify-center`}
                  >
                    <div className="text-center space-y-2 w-full">
                      <div
                        className={`font-mono text-xs font-bold ${
                          isDarkMode ? "text-white/90" : "text-current"
                        } tracking-wider`}
                      >
                        {unit.code}
                      </div>
                      ...
                      <div className="flex justify-center">
                        <Badge
                          className={`text-xs font-bold ${
                            theme.statusBadges[unit.status]
                          } px-3 py-1 shadow-sm`}
                        >
                          {unit.area}
                        </Badge>
                    ...
                    <div
                      className={`absolute top-2 right-2 w-3 h-3 rounded-full ${
                        theme.statusIndicators[unit.status]
                      } animate-pulse`}
                    />
            ...
            >
              <div className="text-sm space-y-1">
                ...
                {unit.isApproved && (
                  <div className="text-blue-400">✓ Đã duyệt</div>
                )}
                {unit.isLocked && (
                  <div className="text-green-400">🔒 Đã khóa</div>
                )}
                {unit.isBlocked && (
                  <div className="text-red-400">🚫 Bị chặn</div>
                )}
              </div>
            </TooltipContent>
          </Tooltip>
            ...
            <TooltipTrigger asChild>
              <DialogTrigger asChild>
                <div
                  ...
                  } relative group hover:scale-[1.03] hover:z-10`}
                >
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span
                        className={`text-xs font-medium ${
                          isDarkMode ? "text-white/80" : "text-current"
                        }`}
                      >
                        Tầng {floor.number}
                      </span>
                  ...
                  <div className="mb-3">
                    ...
                    <div
                      className={`text-xs font-medium ${
                        isDarkMode ? "text-white/70" : "text-current/70"
                      }`}
                    >
      ...
      {viewMode === "list" && (
        <div
          className={`px-6 py-4 ${
            isDarkMode ? "hover:bg-slate-700/20" : "hover:bg-gray-50/50"
          } transition-colors`}
        >
