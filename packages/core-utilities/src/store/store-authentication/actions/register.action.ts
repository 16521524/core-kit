...
export const registerAction = async (data: IRegisterData): Promise<boolean> => {
  ...
  try {
    if (data.password !== data.confirmPassword) {
      authenticationStore.setState({ isFetchingAuthentication: false, fetchingAuthenticationError: 'Passwords do not match' });
      return false;
    }
    ...
    authenticationStore.setState({ isFetchingAuthentication: false, fetchingAuthenticationError: 'An error occurred during registration' });
    return false;
  }
};
