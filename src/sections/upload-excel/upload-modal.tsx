...
export function UploadModal({
      ...
      reader.onload = async (e) => {
        try {
          ...
          if (filteredData.length === 0) {
            setUploadError(
              "Excel file contains no data or is incorrectly formatted."
            );
            setIsLoading(false);
            setUploadStep("error");
            return;
          ...
          const mappedData: ParsedRow[] = filteredData.map((row: any) => {
            ...
            currentSchema.forEach((col) => {
              ...
              const foundKey = Object.keys(row).find(
                (key) => normalizeString(key) === normalizeString(col.name)
              );
          ...
          if (mappedData.length === 0) {
            setUploadError(
              "Excel file contains no data after mapping or is incorrectly formatted."
            );
            setIsLoading(false);
            setUploadStep("error");
            return;
          ...
          const simulatedStatuses: ("Success" | "Error" | "Warning")[] =
    ...
    return parsedData.map((row, rowIndex) => (
      <TableRow key={rowIndex}>
        ...
        <TableCell className="whitespace-nowrap">
          <Badge
            variant={getStatusBadgeVariant(recordStatuses[rowIndex])}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div
        className={`relative w-full max-w-[90vw] h-[90vh] flex flex-col rounded-lg shadow-xl ${theme.primary} ${theme.textPrimary}`}
      >
        {/* Header */}
        <div
          className={`p-6 pb-4 border-b sticky top-0 z-10 rounded-t-lg ${theme.secondary} ${theme.border}`}
        >
          <h2 className="text-2xl font-bold">Upload Excel Data</h2>
          <p className={`${theme.textMuted}`}>
            {uploadStep === "upload" &&
              "Select Excel/CSV file and data type to upload."}
            {uploadStep === "confirm" &&
              "Review record status and confirm save."}
            {uploadStep === "success" &&
              "Data uploaded and saved successfully!"}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-end mt-4">
            <div className="grid w-full items-center gap-2">
              ...
              <div className="relative flex flex-col sm:flex-row sm:items-stretch">
                ...
                />
                <div
                  className={`w-full sm:flex-1 border rounded-t-md sm:rounded-l-md sm:rounded-tr-none sm:rounded-tl-md px-3 py-2 text-sm h-10 flex items-center overflow-hidden whitespace-nowrap text-ellipsis ${theme.border} ${theme.textMuted} ${theme.secondary}`}
                >
                ...
                <Button
                  type="button"
                  variant="outline"
                  className={`w-full sm:w-auto sm:rounded-l-none sm:border-l bg-transparent h-10 border-t-0 sm:border-t
                    ${theme.secondary} 
            ...
            <div className="grid w-full items-center gap-2">
              ...
              <Select value={uploadType} onValueChange={handleUploadTypeChange}>
                <SelectTrigger
                  id="upload-type"
                  className={`h-10 ${theme.secondary} ${theme.border}`}
                >
                  <SelectValue placeholder="Select upload type" />
                </SelectTrigger>
                <SelectContent
                  className={`${theme.secondary} ${theme.textPrimary}`}
                >
        ...
        <div className="flex-1 p-6">
          {isLoading && uploadStep === "upload" && (
            <div className="flex flex-col items-center justify-center h-full text-center py-20 space-y-4">
              ...
              <h3 className={`text-xl font-bold ${theme.textMuted}`}>
                Processing your file...
              </h3>
              <p className={theme.textMuted}>This may take a moment.</p>
          ...
          {uploadStep === "upload" && !isLoading && (
            <div className="space-y-6 max-w-2xl mx-auto py-10">
              <p className={`text-center mb-6 ${theme.textMuted}`}>
                Please download the Excel template, fill in the data, and upload
                it here.
              </p>
            </div>
          )}
          ...
          {(uploadStep === "confirm" ||
            (parsedData.length > 0 &&
              !["upload", "success", "error"].includes(uploadStep))) && (
            <div
              className={`border rounded-lg overflow-hidden shadow-sm h-full max-h-[calc(90vh-280px)] ${theme.border}`}
            >
              <div className="relative w-full overflow-y-auto h-full">
                <Table>
                  <TableHeader
                    className={`sticky top-0 z-10 ${theme.secondary}`}
                  >
                    <TableRow>
                      {tableHeaders.map((header: string) => (
                        <TableHead
                          key={header}
                          className={`${theme.textMuted} whitespace-nowrap`}
                        >
                          {header}
                        </TableHead>
                      ))}
          ...
          {uploadStep === "success" && (
            <div className="flex flex-col items-center justify-center h-full text-center py-20">
              ...
              <p className={`text-lg ${theme.textMuted}`}>
                Your data has been processed and stored.
              </p>
            </div>
          )}
        ...
        {/* Footer */}
        <div
          className={`p-6 pt-4 border-t sticky bottom-0 z-10 rounded-b-lg flex justify-between items-center ${theme.secondary} ${theme.border}`}
        >
          <Button
            variant="outline"
            onClick={handleCloseModal}
