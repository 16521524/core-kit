...
const THEMES = {
  dark: {
    ...
    statusBadges: {
      available:
        "bg-emerald-500/20 text-emerald-200 border border-emerald-400/30 font-semibold",
      ...
      unavailable:
        "bg-gray-500/20 text-gray-200 border border-gray-400/30 font-semibold",
    },
  ...
  light: {
    ...
    statusBadges: {
      available:
        "bg-emerald-300 text-emerald-900 border border-emerald-400 font-semibold",
      sold: "bg-rose-300 text-rose-900 border border-rose-400 font-semibold",
      booked:
        "bg-amber-300 text-amber-900 border border-amber-400 font-semibold",
      unavailable:
        "bg-gray-300 text-gray-900 border border-gray-400 font-semibold",
    },
...
export function ApartmentStackingPlan() {
  ...
  if (loading.overall && !projects.length) {
    return (
      <TooltipProvider>
        <div
          className={`min-h-screen ${currentTheme.primary} ${currentTheme.textPrimary} font-sans transition-colors duration-300`}
        >
          {/* Navigation - Ultra Compact */}
          <div
            className={`${currentTheme.secondary} ${currentTheme.borderLight} border-b px-6 py-1.5 shadow-sm`}
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-8">
                <div className="flex items-center gap-1">
                  <div
                    className={`w-2 h-2 rounded-full ${currentTheme.accent}`}
                  />
                  <span
              ...
              <Button
                ...
                } transition-colors shadow-sm h-8`}
              >
                {theme === "dark" ? (
          ...
          <div
            className={`${currentTheme.secondary} ${currentTheme.borderLight} border-b px-6 py-1.5`}
          >
  ...
  if (error) {
    return (
      <div
        className={`min-h-screen ${currentTheme.primary} ${currentTheme.textPrimary} flex items-center justify-center`}
      >
    ...
    <TooltipProvider>
      ...
      >
            ...
            <div className="flex items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <Select defaultValue="vn">
                  <SelectTrigger
                    className={`w-28 h-8 ${theme === 'dark' ? "bg-slate-700 border-slate-600" : "bg-white border-gray-300"} text-sm shadow-sm`}
                  >
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent className={theme === 'dark' ? "bg-slate-700 border-slate-600" : "bg-white border-gray-300"}>
                ...
                <Select value={priceType} onValueChange={setPriceType}>
                  <SelectTrigger
                    className={`w-32 h-8 ${theme === 'dark' ? "bg-slate-700 border-slate-600" : "bg-white border-gray-300"} text-sm shadow-sm`}
                  >
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent className={theme === 'dark' ? "bg-slate-700 border-slate-600" : "bg-white border-gray-300"}>
          ...
          {loading.overall && !projects.length ? (
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-5 text-sm">
                <span
                  className={`${currentTheme.textMuted} font-medium flex items-center gap-2`}
                >
                ...
                {Object.entries(statusLabels).map(([status, label]) => (
                  <div key={status} className="flex items-center gap-1.5">
                    <div
                      className={`w-2.5 h-2.5 rounded-full ${
                        currentTheme.statusIndicators[
                          status as keyof typeof currentTheme.statusIndicators
                        ]
                      }`}
                    />
                    <span
                      className={`${currentTheme.textSecondary} font-medium text-sm`}
                    >
                      {label}
                    </span>
                  </div>
                ...
                {/* <Button
                  ...
                  className={`relative px-3 py-1.5 rounded-md font-medium text-sm transition-all duration-200 h-8 ${
                    viewMode === "grid"
                      ...
                      : `bg-transparent ${theme === 'dark' ? "hover:bg-slate-600 text-slate-300 hover:text-white" : "hover:bg-white text-gray-600 hover:text-gray-900"} hover:shadow-sm`
                  }`}
                >
        ...
        {loading.content && filteredData.length === 0 ? (
          ...
          <div
            className={`relative ${currentTheme.primary} h-[calc(100vh-375px)] overflow-auto`}
          >
            <div className="min-w-max">
              {/* Fixed Header - Both Vertical and Horizontal Scroll */}
              <div
                className={`sticky top-0 z-30 ${currentTheme.secondary} ${currentTheme.borderLight} border-b shadow-sm`}
              >
                <div className="flex">
                  {/* Fixed Corner Cell */}
                  <div
                    className={`sticky left-0 z-40 ${currentTheme.secondary} ${currentTheme.border} border-r p-3 w-32 flex flex-col items-center justify-center`}
                  >
                    <div
                      className={`font-semibold text-sm ${currentTheme.textPrimary}`}
                    >
                      Floor
                    </div>
                    <div
                      className={`text-xs ${currentTheme.textMuted} font-medium`}
                    >
                      Available
                    </div>
                  </div>
                  ...
                  {blockMetadata?.unit_numbers.map((unitInfo, index) => (
                    <div
                      ...
                      className={`p-3 w-28 text-center ${currentTheme.border} border-r last:border-r-0 ${currentTheme.secondary} flex flex-col items-center justify-center min-w-[112px] max-w-[112px]`}
                    >
                      <div
                        className={`font-semibold text-sm ${currentTheme.textPrimary}`}
                      >
              ...
              {/* Table Body */}
              <div className={`divide-y ${currentTheme.borderLightRow}`}>
                {filteredData.map((floor, floorIndex) => (
                  <div
                    key={`${floor.block}-${floor.number}`}
                    className={`flex min-h-[110px] ${
                      theme === "dark"
                  ...
                  >
                    {/* Fixed Floor Column */}
                    <div
                      className={`sticky left-0 z-20 ${currentTheme.secondary} ${currentTheme.border} border-r p-3 w-32 flex flex-col items-center justify-center min-h-[110px]`}
                    >
                      <div
                        className={`font-bold text-lg ${currentTheme.textPrimary}`}
                      >
                        {floor.floorString || floor.number}
                      </div>
                      <div className="text-xs text-emerald-500 font-medium">
                        {floor.availableCount} units
                      </div>
                      <div
                        className={`text-xs ${currentTheme.textMuted} font-medium`}
                      >
          ...
          // List View
          <div
            className={`${currentTheme.primary} h-[calc(100vh-275px)] overflow-auto`}
          >
            <div
              className={`sticky top-0 z-20 ${currentTheme.secondary} ${currentTheme.borderLight} border-b px-6 py-4 shadow-sm`}
            >
              <div
          ...
          // Grid View
          <div
            className={`p-6 ${currentTheme.primary} h-[calc(100vh-275px)] overflow-auto`}
          >
        ...
        {/* Summary Stats */}
        <div
          className={`${currentTheme.borderLight} border-t ${currentTheme.secondary} p-6 shadow-sm`}
        >
          <div className="flex items-center justify-between text-sm">
            <div className="flex items-center gap-8">
              ...
              <div className="font-medium">
                ...
                <span className={`font-bold ${currentTheme.accentText}`}>
                  BLOCK{" "}
                  {blocks.find((b) => b.id === selectedBlockId)?.name || "A"}
                </span>
              </div>
              <div className="font-medium">
                Number of floors:{" "}
                <span className="font-bold text-amber-500">
                  {blockMetadata?.total_floors || filteredData.length}
                </span>
              </div>
            </div>
            <div className={`${currentTheme.textMuted} font-medium`}>
